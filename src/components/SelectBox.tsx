...
export const SelectBox: FC<any> = ({ label, icon, value, onSelect, variant, key }) => {
  ...
  let selectBoxContainerStyle = {
   ...
   cursor:'pointer',
    ...(isSelected && { bg: '#FFF8F7' }),
    border: `1px solid ${isSelected ? ' #FE7146' : 'rgba(0, 0, 0, 0)'}`
  };
  ...
  return (
    <Flex key={key} onClick={() => onSelect(label)} sx={{ ...selectBoxContainerStyle, ...(variant === 'side' ? variantStyle : { flexDir: 'column' }) }} p={variant !== 'side' ? '27px 20px' : "17px 20px"} pos={'relative'}  >
      {
        icon && (
      ...
      <Text fontWeight={500} fontSize={'18px'} color={'#262626'}  >{label}</Text>
